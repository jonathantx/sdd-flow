...
> **Ancore a SPEC nas skills de stack ativas.** Para cada skill listada acima, consulte `skills/stacks/<skill>/references/architecture.md` (e os demais references pertinentes) antes de propor arquitetura, escolha de libs ou padrões. A SPEC deve produzir código alinhado à base de conhecimento da stack — não invente padrões que contrariem a skill. Se nenhuma skill estiver ativa, sugira rodar `/analyze` primeiro.
