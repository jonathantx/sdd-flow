...
- **`docs/patterns/<stack>/`** — real examples extracted from the existing code
  (controller, service, component, test…), so the agent imitates *this team's*
  style instead of inventing a new one. This is the single biggest reduction in
  hallucination, because stack skills only teach *universal* good practice — the
  patterns teach *how you actually write here*.
...
5. **Extract real patterns** into `docs/patterns/<stack>/` (skip in a new/empty
   project): find one good real example of each artifact type per stack
   (controller, service, repository, test, component, store…), and write a short
   pattern file with the skeleton and the conventions actually observed. Only
   record what you verified in the code — never invent an example.
6. Tell the user to run `sdd sync` to propagate, and `sdd doctor` to validate.
