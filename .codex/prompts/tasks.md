...
- Skills de stack disponíveis: !`ls -1 .sdd/workflow/skills/stacks 2>/dev/null || ls -1 .claude/skills/stacks 2>/dev/null | grep -v README || echo "(nenhuma)"`
...
> **Declare as skills necessárias por task.** Cada task deve listar, no seu frontmatter (campo `skills:`), as skills de stack que o `/implement` precisará carregar para executá-la sem alucinar — escolha entre as stacks ativas, conforme os arquivos que a task toca (ex.: task de UI → `react`/`svelte`; task de API/persistência → `node-typescript`/`php-laravel`). Se nenhuma extra for necessária, use `skills: []`.
