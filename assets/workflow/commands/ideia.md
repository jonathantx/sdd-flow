...
- **chore** — nenhuma decisão de produto, nenhuma ou trivial decisão técnica (só execução). NÃO muda comportamento observável (deps, config interna, rename trivial). → fluxo mínimo (commit direto, ou NOTE.md/ADR se houver memória futura relevante).
- **refactor** — subtipo de chore que **mexe na estrutura do código** sem mudar comportamento (extrair função, remover duplicação, reduzir acoplamento). Não muda o que o usuário vê, mas tem risco de regressão. → fluxo `/refactor` (rede de testes + REFACTOR.md), porque precisa de garantia explícita de que nada quebrou. Use `kind: chore` no frontmatter, mas registre na idea que o caminho é `/refactor`.
...
- Palavras como "atualizar", "bump", "remover não-usado", "renomear", "mover", "migrar versão" → palpite **chore**
- Palavras como "refatorar", "extrair", "limpar o código", "reduzir duplicação", "reorganizar", "simplificar" → palpite **refactor** (→ `/refactor`)
