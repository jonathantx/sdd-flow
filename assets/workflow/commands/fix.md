---
description: Fluxo direto de correção — diagnostica a causa raiz, gera o FIX.md (plano + rollback + teste de regressão) e implementa. Sem PRD, sem decomposição.
argument-hint: <slug-do-fix>
...
Antes de diagnosticar a fundo e antes de escrever a correção/teste, carregue as skills das stacks ativas (listadas acima): leia `skills/stacks/<skill>/SKILL.md` e os `references/` pertinentes ao código tocado (ex.: `persistence.md` para migrations, `testing.md` para o teste de regressão, `api.md` para handlers). Use o **test runner e os comandos da stack** definidos pela skill — nunca assuma uma stack. Sem skill ativa, sugira rodar `/analyze`.
...
## files_touched
- {caminho/exato — extensão conforme a stack do projeto}
- {…}
...
1. **Branch.** Crie/entre em `fix/{slug}` a partir da main atualizada:
   ...
   Se o fix toca banco/migrations, use o comando de migração **da stack ativa** (veja a skill carregada — ex.: `npm run migrate`, `php artisan migrate`, etc.). Não assuma uma stack; siga o que a skill de stack define em `references/persistence.md`.
...
Este comando é a fonte-única `.sdd/workflow/commands/fix.md`. Após `sdd sync`, vira `/fix` no Claude (`.claude/commands/`), Codex (`.codex/prompts/`) e Gemini (`.gemini/commands/`). Invoque com `/fix <slug>`.
